...
- **Score (lavere er bedre):** `tid + fejl×300 + noter + hints×60`. Hjælp-kontakten får uret til at løbe dobbelt så hurtigt (allerede i timer-intervallet). `noteCount` er akkumulerede note-strafsekunder: **1 s pr. enkelt note**, **10 s pr. felt** ved dobbelttryk/højreklik der udfylder alle kandidater (ikke 1× antal tal). Sletning refunderes ikke. Hvert hint-tryk (pære) koster 60 s. Manglende `noteCount`/`hintCount` på ældre poster tæller som 0 — eksisterende Blobs-rækker og store-navnet `sudoku-highscores` røres ikke; nye scores bruger den nye formel.
